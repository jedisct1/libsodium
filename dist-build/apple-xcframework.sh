...
export TVOS_SIMULATOR_I386_PREFIX="${PREFIX}/tmp/tvos-simulator-i386"
...
build_tvos_simulator() {
  ...
  ## i386 simulator
  export CFLAGS="-O2 -arch i386 -isysroot ${SDK} -mtvos-simulator-version-min=${TVOS_SIMULATOR_VERSION_MIN}"
  export LDFLAGS="-arch i386 -isysroot ${SDK} -mtvos-simulator-version-min=${TVOS_SIMULATOR_VERSION_MIN}"

  make distclean >/dev/null 2>&1
  ./configure --host=i686-apple-darwin10 --prefix="$TVOS_SIMULATOR_I386_PREFIX" \
    ${LIBSODIUM_ENABLE_MINIMAL_FLAG} || exit 1
  make -j${PROCESSORS} install || exit 1
...
for ext in a dylib; do
  if [ "$APPLE_SILICON_SUPPORTED" = "true" ]; then
    lipo -create \
      ...
      "${TVOS_SIMULATOR_I386_PREFIX}/lib/libsodium.${ext}" \

#define TEST_NAME "aead_aegis256x2"
#include "cmptest.h"
...
static struct {
    ...
    { "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f",
      "101112131415161718191a1b1c1d1e1f202122232425262728292a2b2c2d2e2f",
      "04050607040506070405060704050607040506070405060704050607040506070405060704050607040506070405"
      "06070405060704050607040506070405060704050607040506070405060704050607040506070405060704050607"
      "04050607040506070405060704050607040506070405060704050607",
      "0102030401020304",
      "72120c2ea8236180d67859001f4729077b7064c414384fe3a7b52f1571f4f8a7d0f01e18db4f3bc0adb150702e5d"
      "147a8d36522132761b994c1bd395589e2ccf0790dfe2a3d12d61cd666b2859827739db4037dd3124c78424459376"
      "f6cac08e1a7223a2a43e398ce6385cd654a19f481cba3b8f25910b42",
...
tv(void)
{
    ...
    key   = (unsigned char *) sodium_malloc(crypto_aead_aegis256x2_KEYBYTES);
    nonce = (unsigned char *) sodium_malloc(crypto_aead_aegis256x2_NPUBBYTES);
    mac   = (unsigned char *) sodium_malloc(crypto_aead_aegis256x2_ABYTES);

    do {
        assert(strlen(tests[i].key_hex) == 2 * crypto_aead_aegis256x2_KEYBYTES);
        sodium_hex2bin(key, crypto_aead_aegis256x2_KEYBYTES, tests[i].key_hex,
                       strlen(tests[i].key_hex), NULL, NULL, NULL);
        assert(strlen(tests[i].nonce_hex) == 2 * crypto_aead_aegis256x2_NPUBBYTES);
        sodium_hex2bin(nonce, crypto_aead_aegis256x2_NPUBBYTES, tests[i].nonce_hex,
        ...
        ciphertext_len          = message_len + crypto_aead_aegis256x2_ABYTES;
        ...
        assert(strlen(tests[i].mac_hex) == 2 * crypto_aead_aegis256x2_ABYTES);
        sodium_hex2bin(expected_ciphertext + message_len, crypto_aead_aegis256x2_ABYTES,
        ...
        crypto_aead_aegis256x2_encrypt_detached(detached_ciphertext, mac, &found_mac_len, message,
                                              message_len, ad, ad_len, NULL, nonce, key);
        assert(found_mac_len == crypto_aead_aegis256x2_ABYTES);
        if (memcmp(detached_ciphertext, expected_ciphertext, detached_ciphertext_len) != 0 ||
            memcmp(mac, expected_ciphertext + message_len, crypto_aead_aegis256x2_ABYTES) != 0) {
        ...
        crypto_aead_aegis256x2_encrypt(ciphertext, &found_ciphertext_len, message, message_len, ad,
        ...
        found_message_len = 1;
        if (crypto_aead_aegis256x2_decrypt(decrypted, &found_message_len, NULL, ciphertext,
        ...
        if (crypto_aead_aegis256x2_decrypt(decrypted, &found_message_len, NULL, guard_page,
                                         randombytes_uniform(crypto_aead_aegis256x2_ABYTES), ad,
                                         ad_len, nonce, key) != -1) {
            printf("Verification of test vector #%u with a truncated tag failed\n",
                   (unsigned int) i);
        }
        if (i == 0 && crypto_aead_aegis256x2_decrypt(NULL, NULL, NULL, ciphertext, ciphertext_len, ad,
                                                   ad_len, nonce, key) != 0) {
        ...
        if (crypto_aead_aegis256x2_decrypt_detached(decrypted, NULL, detached_ciphertext,
...
{
    tv();
    assert(crypto_aead_aegis256x2_keybytes() == crypto_aead_aegis256x2_KEYBYTES);
    assert(crypto_aead_aegis256x2_nsecbytes() == crypto_aead_aegis256x2_NSECBYTES);
    assert(crypto_aead_aegis256x2_npubbytes() == crypto_aead_aegis256x2_NPUBBYTES);
    assert(crypto_aead_aegis256x2_abytes() == crypto_aead_aegis256x2_ABYTES);
    assert(crypto_aead_aegis256x2_messagebytes_max() == crypto_aead_aegis256x2_MESSAGEBYTES_MAX);

    {
        unsigned char k[crypto_aead_aegis256x2_KEYBYTES];
        crypto_aead_aegis256x2_keygen(k);
